...
class DashboardSummaryView(APIView):
    ...
    def get(self, request):
        ...
        average_response = (
            SOS.objects.filter(status__in=["acknowledged", "resolved"])
            .annotate(
        ...
        escalated_count = SOS.objects.filter(escalation_logs__isnull=False).distinct().count()
        volunteer_responding = SOS.objects.filter(status="acknowledged").count()
        pending_guardian = SOS.objects.filter(status="pending").count()
        ...
        return Response({
            ...
            "category_counts": [
                {"category": item["name"] or "Uncategorized", "count": item["count"]}
